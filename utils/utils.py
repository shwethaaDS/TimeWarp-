...
def plot_pairwise(root_dir,root_dir_dataset_archive, classifier_name_1, classifier_name_2,
    ...
    data_name = ""
    for dataset_name in df_data.index:  # Iterate through dataset names
        data_name += dataset_name + "_"
        plt.scatter(x=df_data.loc[dataset_name, classifier_name_1], 
                    y=df_data.loc[dataset_name, classifier_name_2], 
                    color=np.random.rand(3,),  # Assign random color
                    label=dataset_name)      # c=sorted_df['theme_colors'])
    plt.xlabel('without data augmentation', fontsize='large')
    plt.ylabel('with data augmentation', fontsize='large')
    plt.plot(x, x, color='black')
    plt.legend(loc='best', fontsize='small', markerscale=0.5)  # Add legend, adjust size, markers
    plt.title(title)
    ...
    plt.savefig(root_dir + '/' + data_name + title + '.pdf'
                , bbox_inches='tight')